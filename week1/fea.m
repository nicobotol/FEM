...
for i=1:size(bound,1)
    ...
    K(:, 2*node - (2 - ldof)) = 0; % putting 0 on the columns
    K(2*node - (2 - ldof), :) = 0; % putting 0 on the rows
    K(2*node - (2 - ldof), 2*node - (2 - ldof)) = 1; % putting 1 in the dyagonal

    P(2*node - (2 - ldof)) = 0; % putting 0 in P
end
