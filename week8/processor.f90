...
!        call system('ls' ) ! Linux
        ...
        do
            ...
            case ('REAL', 'real')
                ! TODO
                ! This command is only partially implemented -- it should not be used (it has no effect at the moment). The command is supposed to set the default value of "R", which is to be used for area or thickness in materials, that do not have this value set explicitly (via a call of "R" command before calling "MP" on some card).
                backspace (10)
                read (10, *) command, ivalue
                currentr = ivalue
            case ('D', 'd')
